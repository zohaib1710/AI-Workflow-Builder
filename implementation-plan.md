...
### V2 Visual Correction: Improve workflow diagram readability

- Status: INCOMPLETE (implementation and automated validation complete; real-workflow browser inspection pending)
- Purpose: Keep generated, reconciled, auto-arranged, created, and manually moved workflow nodes understandable and free from visual overlap.
- Files modified: shape registry and node renderer, Dagre layout and presentation placement, reconciliation, active and legacy canvas components, editor canvas styles, and focused layout/canvas tests.
- Implementation: Dagre now receives actual shape dimensions, larger rank/branch/edge spacing, and edge-label dimensions before a deterministic collision-separation pass. Auto Arrange respects current shape overrides; AI reconciliation, node creation, and manual drag completion resolve occupied placement without changing annotations. Shape bounds were enlarged and text remains clipped with accessible full-value titles. Fit-all uses asymmetric overlay-safe padding, and the MiniMap is explicitly anchored to the desktop bottom-right. Initial generation fit is guarded to run once per canvas mount, preventing node or annotation synchronization from refitting the viewport. Entering browser or Fullscreen API fullscreen still waits two animation frames for the resized canvas, then performs one viewport-only fit over visible semantic workflow nodes at a 0.05 minimum zoom; exit and later interaction do not refit.
- Required validation: focused layout/canvas suites; frontend production build; `git diff --check`; manual browser inspection of one representative complex workflow.
- Acceptance criteria:
  - [x] Focused layout/canvas tests prove deterministic placement, shape-aware geometry, no rectangle intersections, collision-safe manual placement, fit-all configuration, and bottom-right MiniMap position.
  - [x] Production build and `git diff --check` pass without backend, schema, dependency, or lock-file changes.
  - [x] Node content is constrained within enlarged shape bounds and full values remain available through accessible titles.
  - [x] Fullscreen entry fits the complete semantic workflow after resize without moving nodes, changing history, or continuously overriding zoom and pan.
  - [x] Node and annotation position commits cannot retrigger initial fit or call viewport-centering APIs.
  - [ ] A representative generated workflow is visually confirmed in a real browser with clear branches, no overlap, and the MiniMap at the bottom-right.
- Commit message: `fix(frontend): improve workflow diagram readability`
- Stop conditions: Stop if readability requires backend coordinates, a new layout dependency, a schema change, or abandoning left-to-right fit-all behavior.
